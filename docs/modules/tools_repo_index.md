...
This file, `repo_index.py`, is part of the Operon v5 system and is responsible for building a multi-layered index of a repository. It includes functions to list repository files, compute file hashes, and build a symbol index using an AST-based parser. The index is designed to support semantic search, dependency resolution, and content-addressed caching, with a new full cross-reference graph added in v5.
